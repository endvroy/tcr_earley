...
class Item:
    ...
    def __init__(self, rule, dot=0, at=0):
        # CFG Rule.
        self.rule = rule
    ...
    def copy(self):
        return Item(self.rule, self.dot, self.at)
    ...
    def __repr__(self):
        ...
        return f'Item({self.rule.lhs} -> {a}*{b}@{self.at})'
...
class EarleyParser:
    ...
    def predict(self, item, pos):
        for rule in self.grammar[item.next_symbol()]:
            self.table[pos].add(Item(rule, dot=0, at=pos))
